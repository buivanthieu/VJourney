...
export default function AdminDashboard() {
  ...
  const loadData = async () => {
    setIsLoading(true);
    try {
      const resLocations = await fetch(`${process.env.NEXT_PUBLIC_API_URL || 'https://localhost:7001/api'}/locations`);
      if (resLocations.ok) {
  ...
  return (
    <div className="bg-white p-6 rounded-xl border border-slate-200 shadow-sm">
          ...
          <div className="md:col-span-2">
            ...
            <div className="bg-white rounded-md overflow-hidden border border-slate-300">
              <ReactQuill 
                ...
                className="h-72 mb-12" // Tạo khoảng trống chiều cao cho khung soạn thảo
              />
            </div>
          </div>
      ...
      {/* BẢNG VIEW DANH SÁCH TOURS (Giữ nguyên cấu trúc bảng) */}
      {isLoading ? (

...
export default function ToursPage() {
  ...
  // GỌI HÀM SẠCH SẼ TỪ TOUR_API ĐÃ ĐƯỢC ĐÓNG GÓI TRUNG TÂM
  useEffect(() => {
    const fetchData = async () => {
      try {
        const [tourData, locData] = await Promise.all([
          tourApi.getAllTours(),
          tourApi.getLocations() // Dùng trực tiếp hàm mới gom gọn
        ]);
  ...
  // LOGIC LỌC (Giữ nguyên xử lý mượt mà phía Client)
      ...
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-8">
        ...
        <div className="lg:col-span-1 bg-white p-6 rounded-xl border border-slate-200 shadow-sm h-fit space-y-6">
          ...
          <div>
            ...
            <select value={selectedLocation} onChange={(e) => setSelectedLocation(e.target.value)} className="w-full rounded-md border border-slate-300 bg-slate-50 px-3 py-2 text-xs outline-none focus:border-emerald-500 focus:bg-white transition-all">
